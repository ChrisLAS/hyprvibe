...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # Hyprland 0.55 prefers hyprland.lua when both config formats exist. Force
    # the declaratively managed Hyprlang config while the Lua migration is
    # still pending.
    environment.sessionVariables.HYPRLAND_CONFIG = "${userHome}/.config/hypr/hyprland.conf";
    ...
    system.activationScripts.hyprlandBase = lib.mkAfter ''
      ...
      # Hyprland 0.56 may have generated this after rejecting the legacy config.
      # Remove only its generated sample; never delete a user-authored Lua file.
      if [ -f ${userHome}/.config/hypr/hyprland.lua ] \
        && ${pkgs.gnugrep}/bin/grep -q 'AUTOGENERATED HYPRLAND CONFIG' ${userHome}/.config/hypr/hyprland.lua; then
    ...
    systemd.user.services.hyprvibe-setup-hyprland = {
      ...
      serviceConfig = {
        ...
        ExecStart = pkgs.writeShellScript "hyprvibe-setup-hyprland" ''
          ...
          mkdir -p ${userHome}/.config/hypr
          # Clean up only the generated Lua fallback left by Hyprland 0.56.
          if [ -f ${userHome}/.config/hypr/hyprland.lua ] \
            && ${pkgs.gnugrep}/bin/grep -q 'AUTOGENERATED HYPRLAND CONFIG' ${userHome}/.config/hypr/hyprland.lua; then
            rm -f ${userHome}/.config/hypr/hyprland.lua
          fi
